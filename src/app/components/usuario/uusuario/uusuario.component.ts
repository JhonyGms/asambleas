...
export class UusuarioComponent implements OnInit {
  imgfondo = '/assets/scpaye rojo.png';
    ...
    if (this.user.asamblea == '') {
      ...
      this.authService.signInUser(this.user).subscribe(
        (res) => {
          ...
          // const dialogref = this.dialog.open(ConfirmDialogComponent,{data:{title:`Bienvenido ${this.user.username}` ,text:"Este es tu apartado para la asamblea"}})
  ...
  changeWebsite(s) {
    if (this.user.asamblea == '') {
      console.log('vacio');
      this.imgfondo = '/assets/scpaye rojo.png';
    } else {
